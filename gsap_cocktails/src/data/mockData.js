...
function extractKeywords(text) {
  const keywordPool = ['孤独', '黄昏', '春天', '月光', '江河', '山林', '青春',
    '远方', '梦境', '时光', '风', '雨', '夜', '城市', '灵魂', '记忆', '影子', '星辰'];
...
function detectEmotion(text) {
  const emotions = ['孤独', '温柔', '激昂', '怀旧', '治愈', '沉思', '忧伤', '宁静'];
  // Simple heuristic based on character matching
  const lower = text || '';
  if (lower.includes('孤独') || lower.includes('寂寞') || lower.includes('绝望')) return '孤独';
  if (lower.includes('春天') || lower.includes('温暖') || lower.includes('温柔')) return '温柔';
  if (lower.includes('激昂') || lower.includes('青春') || lower.includes('奋斗')) return '激昂';
  if (lower.includes('记忆') || lower.includes('往事') || lower.includes('怀念')) return '怀旧';
  if (lower.includes('治愈') || lower.includes('希望') || lower.includes('光亮')) return '治愈';
...
export const agentSteps = [
  ...
  },
  {
    ...
    output: '相似作品列表 + 相似度评分',
    status: 'running',
    detail: '在古典诗词和现代诗歌知识库中并行搜索，返回最匹配的结果。',
  },
  {
    id: 'style',
    name: 'StyleAgent',
  ...
  },
  {
    ...
    output: '生成作品 / 润色后文本',
    status: 'pending',
    detail: '融合意图、情感、关键词、参考作品和推荐风格，进行创作。',
  },
  {
    id: 'summary',
    name: 'SummaryAgent',
